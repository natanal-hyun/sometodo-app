...
### 🙋‍♂️ 느낀 점
이번 프로젝트를 통해 계정 인증 흐름에 대해 처음부터 끝까지 직접 구현해보며, **실제 서비스에서 사용되는 인증 방식**을 경험할 수 있었습니다. 단순히 로그인만 처리하는 수준이 아니라, JWT 기반의 인증과 토큰 만료 및 갱신, 그리고 인증 후 사용자 식별까지 전반적인 구조를 이해하고 적용할 수 있었습니다.

### 🔧 어려웠던 점
처음에는 여러 라이브러리를 사용하면서 예상치 못한 오류나 의존성 충돌이 자주 발생했습니다. 특히, 외부 API(Google OAuth)를 사용할 때는 구글 콘솔 설정이나 토큰 처리 과정에서 많은 시행착오를 겪었습니다. 하지만 디버깅을 통해 차근차근 문제를 해결하며 **문제 해결 능력**이 크게 향상되었습니다.

### 🤝 협업 경험
팀원들과 역할을 나누어 기능을 나눠서 개발하고, 이후 통합하며 서로의 코드를 확인하고 이해하는 과정에서 **코드 스타일이나 구조에 대한 이해도**가 높아졌습니다. 특히 공통된 API 요청 방식, 응답 처리 구조를 정하면서 **협업을 위한 코드 작성 습관**이 생겼습니다.

### 🌱 아쉬운 점 & 성장 포인트
계정 파트 외에도 구현하고 싶었던 기능(예: 그룹 일정 등)이 있었지만, 시간 관계상 미뤄야 했던 점은 아쉽습니다. 그럼에도 불구하고 프로젝트 전체 흐름을 이해하며 백엔드와 프론트를 동시에 경험한 점은 큰 성장으로 느껴졌습니다.
